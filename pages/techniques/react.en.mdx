...
Chain-of-thought (CoT) prompting has shown the capabilities of LLMs to carry out reasoning traces to generate answers to questions involving arithmetic and commonsense reasoning, among other tasks [(Wei et al., 2022)](https://arxiv.org/abs/2201.11903). But the lack of access to the external world or inability to update its knowledge can lead to issues like fact hallucination and error propagation. 
...
Here is an example of what the ReAct prompt exemplars looks like (obtained from the paper and shortened to one example for simplicity): 
